...
for(table in tables){
  if(is.null(chunkSize)){
    ...
    for(rowOff in 0:(totalRows %/% chunkSize)) {
      ...
      dtP = data.table::as.data.table(DBI::dbGetQuery(pCon, glue('SELECT * FROM {`table`} ORDER BY {`colOrder`} LIMIT {`chunkSize`} OFFSET {`off`}')))
      setorder(dtP)

      dtB = data.table::as.data.table(bq_dataset_query(bq_dataset(project, dataset), glue('SELECT * FROM {`table`} ORDER BY {`colOrder`} LIMIT {`chunkSize`} OFFSET {`off`}')))
      setorder(dtB)
